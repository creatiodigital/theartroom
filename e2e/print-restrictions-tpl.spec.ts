...
import { TPL_PAPERS } from '@/lib/print-providers/tpl/data'
 ...
 *
 * Sizes are intentionally not asserted: TPL exposes a continuous size
 * slider rather than preset checkboxes, so there is no veto surface
 * to test. Paper veto is the relevant contract here.
 */

test.skip('TPL: wizard hides papers the artist vetoed in the DB', async ({ page }) => {
  ...
  const vetoedPaper = TPL_PAPERS[TPL_PAPERS.length - 1]
  const seededAllowedPapers = TPL_PAPERS.filter((p) => p.id !== vetoedPaper.id).map((p) => p.id)
  ...
  try {
    ...
    // TPL bundles printType + paper into a single "Print" section with two
    ...
    expect(printTypeLabels.length, 'TPL wizard should expose ≥1 print type').toBeGreaterThan(0)
    ...
    expect(seenPapers.size, 'TPL wizard should expose ≥1 paper across print types').toBeGreaterThan(
      0,
    )
  } finally {
